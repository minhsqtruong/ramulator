...
5) HPFRFCFS - Minimalist Open Page: Higher Priority Request First, priority is
given by memory read requests priority assignment using MLP score generated by
MSHR at L2 for normal requests and prefetch depth for prefeth request. Check if
multi-line request is READY, if yes then commit. If multiple requests satifies
the same conditions for the previous 2 filters, then scheduled chronologically
...
public:
    ...
    list<Request>::iterator get_head(list<Request>& q)
    {
        ...
        if (type != Type::FRFCFS_PriorHit || type != Type::HPFRFCFS) {
        ...
        // Minh: Minimalist Open Page uses the same scheme as FRFCFS_PriorHit
...
private:
    ...
    function<ReqIter(ReqIter, ReqIter)> compare[int(Type::MAX)] = {
        ...
        // HPFRFCFS
        [this] (ReqIter req1, ReqIter req2) {
          // Priority only applies to read request, revert to FRFCFS_PriorHit otherwise
          if (req1->priority > req2->priority
              && req1->type == Request::Type::READ
              && req2->type == Request::Type::READ) return req1;
          else if(req1->priority < req2->priority
              && req1->type == Request::Type::READ
              && req2->type == Request::Type::READ) return req2;
          else {
            bool ready1 = this->ctrl->is_ready(req1) && this->ctrl->is_row_hit(req1);
            bool ready2 = this->ctrl->is_ready(req2) && this->ctrl->is_row_hit(req2);
